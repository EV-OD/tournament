...
import { computeAmountsFromBooking, computeAmountsFromVenue } from '@/lib/pricing';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!venueId || !date || !startTime) {
      return NextResponse.json({ error: 'Missing venueId, date or startTime' }, { status: 400 });
    }
    ...
    if (!venueSlots) {
      return NextResponse.json({ error: 'Venue slots not initialized' }, { status: 404 });
    }
    ...
    const checkSingle = (checkDate: string, checkStart: string) => {
      // Check blocked
      if ((venueSlots.blocked || []).some((b) => b.date === checkDate && b.startTime === checkStart)) {
        conflicts.push('blocked');
        return;
      }
      // Check bookings
      if ((venueSlots.bookings || []).some((b) => b.date === checkDate && b.startTime === checkStart)) {
        conflicts.push('booked');
        return;
      }
      // Check held (ensure not expired)
      const heldSlot = (venueSlots.held || []).find((h) => h.date === checkDate && h.startTime === checkStart);
      if (heldSlot) {
        const now = new Date();
        const expiresAt = heldSlot.holdExpiresAt?.toDate ? heldSlot.holdExpiresAt.toDate() : (heldSlot.holdExpiresAt ? new Date(heldSlot.holdExpiresAt) : null);
      ...
      // Check reserved
      if ((venueSlots.reserved || []).some((r) => r.date === checkDate && r.startTime === checkStart)) {
    ...
    const computed = computeAmountsFromVenue(pricePerHour, slotDuration, slotsCount);

    return NextResponse.json({ success: true, available, conflicts, computed, slotDuration, slotsCount });
  } catch (error) {
    console.error('Error computing amount:', error);
    return NextResponse.json({ error: 'Failed to compute amount' }, { status: 500 });
  }
}
